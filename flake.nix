    ...
    in
    {
      ...
      let
        ...
        myx = pkgs.writeShellScriptBin "myx" ''
          ...
          exec env DISPLAY=:1 PATH="''${PATH:+''${PATH}:}${pkgs.dbus}/bin:${pkgs.kdePackages.kwin}/bin:${pkgs.plasma5Packages.plasma-workspace}/bin:${pkgs.plasma5Packages.plasma-desktop}/bin:${pkgs.lib.getBin pkgs.plasma5Packages.kinit}/libexec/kf5" ${pkgs.dbus}/bin/dbus-run-session --config-file=${pkgs.dbus}/share/dbus-1/session.conf ${pkgs.plasma5Packages.plasma-workspace}/bin/startplasma-x11 &
