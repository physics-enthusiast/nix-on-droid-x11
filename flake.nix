    ...
    in
    {
      ...
      let
        ...
        myx = pkgs.writeShellScriptBin "myx" ''
          ...
          exec env PATH="''${PATH:+''${PATH}:}${pkgs.dbus}/bin" ${pkgs.dbus}/bin/dbus-run-session --config-file=${pkgs.dbus}/share/dbus-1/session.conf ${pkgs.plasma5Packages.kwin}/bin/kwin_x11 &
